...
def get_ai_client():
    return LlmChat(
        ...
        session_id=f"ats-extraction-{datetime.now().timestamp()}",
        system_message="You are an expert at extracting structured data from CVs and resumes."
...
app.add_middleware(
    ...
    # Cargaremos los orígenes permitidos desde una variable de entorno
    allow_origins=os.environ.get('CORS_ORIGINS', 'http://localhost:3000').split(','),
    allow_methods=["*"],
    allow_headers=["*"],
)

# Notion Database Options (from analysis)
NOTION_OPTIONS = {
    "STAGE": [
...
async def extract_linkedin_data(linkedin_url: str) -> str:
    """
    Extrae datos básicos de LinkedIn usando web scraping simple
    """
    try:
        headers = {
...
async def extract_skills_with_ai(text_content: str) -> List[str]:
    """
    Extrae skills usando IA con categorías específicas y límites
    """
    try:
        prompt = f"""
...
"""
        ...
        # Combinar todas las categorías en una lista final
        final_skills = []
        ...
        return final_skills[:8]  # Máximo 8 skills total
...
async def extract_skills_basic(text_content: str) -> List[str]:
    """
    Extracción básica de skills como fallback
    """
    skill_keywords = [
...
async def extract_name_advanced(text_content: str) -> str:
    """
    Extracción avanzada de nombre usando IA
    """
    try:
        prompt = f"""
...
3. NO incluyas texto como "Completar manually" o "Highly Confidential"
...
async def extract_phone_advanced(text_content: str) -> str:
    """
    Extracción avanzada de teléfono con formato correcto
    """
    ...
    phone_patterns = [
        ...
        r'(\+1\s*[2-9][0-9]{2}\s*[0-9]{3}\s*[0-9]{4})',  # USA/Canadá
        r'(\+[1-9][0-9]{1,3}\s*[0-9]{6,12})',            # Internacional general
        # Números nacionales
        r'\b([6-9][0-9]{8})\b',  # España sin prefijo
        r'\b(\([2-9][0-9]{2}\)\s*[0-9]{3}[-\s]*[0-9]{4})\b'  # USA sin prefijo
    ]
    ...
    for pattern in phone_patterns:
        ...
        if matches:
            ...
            if len(clean_phone) >= 9:
                ...
                # Número válido sin prefijo, asumir España si es europeo
...
async def extract_linkedin_advanced(text_content: str) -> str:
    """
    Extracción ultra mejorada de LinkedIn URL incluyendo links incrustados en palabras
    """
    # 1. Buscar patrones directos en texto
    linkedin_patterns = [
    ...
    # 2. Buscar en HTML/XML - links incrustados en tags
    try:
        soup = BeautifulSoup(text_content, 'html.parser')
        
        # Buscar links con href que contenga linkedin
        links = soup.find_all('a', href=True)
        for link in links:
            href = link.get('href', '')
            text = link.get_text().lower()
            
            # Si el texto es "linkedin" y tiene un href válido
        ...
        # Buscar cualquier elemento que contenga linkedin URL
    ...
    # 3. Buscar en texto plano con contexto de "LinkedIn"
    lines = text_content.split('\n')
    for line in lines:
        line_lower = line.lower()
        
        # Si la línea menciona linkedin, buscar URLs cerca
...
async def extract_company_advanced(text_content: str) -> str:
    """
    Extracción avanzada de empresa actual usando IA
    """
    try:
        prompt = f"""
...
async def extract_languages_advanced(text_content: str) -> List[str]:
    """
    Extracción avanzada de idiomas con preselección
    """
    detected_languages = []
    text_lower = text_content.lower()
    
    # Patrones específicos para idiomas con niveles
    language_patterns = {
    ...
    # Si no detectó idiomas, usar heurísticas
    if not detected_languages:
        # Si el CV está en español, asumir español nativo
        ...
        # Si menciona inglés, asumir nivel intermedio
...
async def extract_gender_advanced(text_content: str, nombre_apellido: str) -> str:
    """
    Extracción avanzada de género usando IA
    """
    try:
        # Usar el nombre como pista principal
        prompt = f"""
...
async def extract_candidate_data_ultra_improved(text_content: str, source: str = "cv") -> CandidateCreate:
    ...
    try:
        ...
        return CandidateCreate(
            ...
            skills=[],
            languages=[],
            gender="" # <--- ¡CAMPO AÑADIDO!
        )

# --- ¡NUEVA FUNCIÓN MEJORADA! ---
# La función 'upload_cv_to_attachment' en tu código tenía un error:
# intentaba subir a 'api.notion.com/v1/files', que no es una API pública
# para adjuntos. La forma correcta es subirlo a un servicio externo
# (como S3) y dar a Notion la URL.
# Para simplificar, usaremos un servicio gratuito de subida temporal.
# NOTA: Estos archivos pueden borrarse después de un tiempo.
async def upload_cv_to_external_service(file_content: bytes, filename: str) -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            # Usamos 'file.io' como un servicio simple de subida.
            # Límite de 1 subida. Válido por 14 días.
            ...
            # Fallback si file.io falla
            logging.error(f"Error subiendo a file.io: {response.text}")
            return "" # Retornar vacío para no bloquear la creación del candidato
...
async def create_notion_record(candidate: CandidateCreate, file_content: bytes = None, filename: str = None) -> tuple[str, str]:
    """
    Crea un registro en Notion con los datos del candidato y adjunta el archivo
    """
    try:
        headers = {
        ...
        # Adjuntar archivo CV si existe - DIRECTAMENTE EN PROPIEDAD ATTACHMENT
        if file_content and filename:
            try:
                # Subir archivo a un servicio temporal y obtener URL
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            # Archivo ya fue agregado en la propiedad ATTACHMENT durante la creación
...
async def get_notion_options():
    """
    Obtiene todas las opciones disponibles para los dropdowns
    """
...
async def upload_cv(file: UploadFile = File(...)):
    """
    Procesa un archivo CV (PDF o DOCX) y extrae los datos del candidato para vista previa
    """
    ...
    try:
        ...
        # Usar la función de extracción ultra mejorada
...
async def process_linkedin(request: LinkedInProcessRequest):
    """
    Procesa una URL de LinkedIn y extrae los datos del candidato para vista previa
    """
    try:
...
async def confirm_create_candidate(request: ConfirmCandidateRequest):
    """
    Confirma la creación del candidato en Notion después de la vista previa
    """
    try:
        ...
        # Crear registro en Notion con attachment
...
async def get_candidates():
    """
    Obtiene la lista de todos los candidatos procesados
    """
    try:
...
async def notion_health_check():
    """
    Verifica la conectividad con la API de Notion
    """
    try:
        headers = {
            "Authorization": f"Bearer {os.environ.get('NOTION_API_TOKEN')}",
