...
class ExtractedData(BaseModel):
    ...
    def clean_and_format_phone_v2(cls, v):
        if not v or not isinstance(v, str): return ""
        clean_phone = re.sub(r'[^\d+]', '', v)
        digits_only = re.sub(r'\D', '', clean_phone)
        if clean_phone.startswith('+') and len(digits_only) >= 9: return f"+{digits_only}"
        elif len(digits_only) == 9 and digits_only[0] in '6789': return f"+34{digits_only}"
        elif len(digits_only) == 10 and digits_only[0] in '23456789': return f"+1{digits_only}"
...
async def extract_linkedin_from_text_robust(text_content: str) -> str:
    patterns = [ r'(https?://(?:www\.)?linkedin\.com/in/[a-zA-Z0-9\-_%/]+)', r'linkedin\.com/in/[a-zA-Z0-9\-_%/]+' ]
    urls_found = set()
    ...
    if urls_found: best_url = list(urls_found)[0]; logger.info(f"LinkedIn por Regex/BS: {best_url}"); return best_url
    logger.info("LinkedIn no por Regex/BS."); return ""

async def extract_data_with_ai(text_content: str) -> ExtractedData:
    if not openai_client: raise HTTPException(status_code=503, detail="Servicio IA no configurado.")
    logger.info("Extracción IA (Prompt v5.1)...")
    ...
    RESPONDE SOLO CON JSON VÁLIDO. SIN ```json NI comentarios.
    {{ "nombre_apellido": "...", ... }}
    """
    try:
        response = await openai_client.chat.completions.create(
            model="gpt-4o-mini", messages=[ {"role": "system", "content": "Extrae datos CV/LinkedIn a JSON."}, {"role": "user", "content": prompt}],
            response_format={"type": "json_object"}, temperature=0.0, max_tokens=1500 )
        json_response = response.choices[0].message.content
        logger.info(f"Respuesta JSON IA: {json_response}")
        ...
        logger.info("Extracción IA completada y validada.")
        return extracted
    except HTTPException: raise
    except Exception as e: logger.exception("Error inesperado extracción IA"); raise HTTPException(status_code=500, detail=f"Error interno IA: {e}")
...
async def check_email_duplicate(email: Optional[EmailStr]) -> DuplicateCheckResponse:
    ...
    except Exception as e: logger.error(f"Error check duplicado email {email}: {e}"); return DuplicateCheckResponse(exists=False)
     ...
     try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if response.status_code == 200:
                ...
                if file_url: logger.info(f"Subido a file.io: {file_url}"); return file_url
                else: logger.error(f"file.io OK pero sin link: {data}"); return ""
            else: logger.error(f"Error subiendo a file.io ({response.status_code}): {response.text}"); return ""
     except Exception as e: logger.error(f"Excepción en upload_cv_to_external_service: {e}"); return ""

async def create_notion_page(candidate_data: CandidateDataInput) -> tuple[str, str]:
    if not NOTION_API_TOKEN or not NOTION_DATABASE_ID: raise HTTPException(status_code=503, detail="Configuración Notion incompleta.")
    ...
    try:
        async with httpx.AsyncClient(timeout=45.0) as client:
            ...
            data = response.json(); logger.info(f"Notion OK: ID={data['id']}"); return data['id'], data['url']
    except httpx.HTTPStatusError as e:
        error_body = e.response.text; logger.error(f"Error API Notion ({e.response.status_code}): {error_body}")
...
async def process_source_endpoint_v3(file: Optional[UploadFile] = File(None), linkedin_url: Optional[HttpUrl] = Form(None)):
    ...
    try:
        ...
        return {"extracted_data": extracted_dict, "file_info": file_info, "source_type": source_type, "duplicate_info": duplicate_dict}
    except HTTPException as e: logger.error(f"HTTPException /process: {e.detail}"); raise e
    except Exception as e: logger.exception("Error inesperado /process"); raise HTTPException(status_code=500, detail=f"Error interno: {e}")

@api_router.post("/candidates/confirm-create", response_model=ConfirmCreateResponse)
async def confirm_create_endpoint_v4(request: ConfirmCreateRequest):
